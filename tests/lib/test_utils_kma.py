...
from src.lib.utils.kma_api_utils import Kma_api_collector
...
# -----------------------------------------------------
# Test get_stn_number method
# -----------------------------------------------------

def test_get_stn_number_success(stn_meta_df, collector):
    result = collector.get_stn_number(stn_meta_df, '서울')
    assert result == 100


def test_get_stn_number_missing_columns(collector):
    df = pd.DataFrame({
        '지역' : ['서울', '인천']
    })

    with pytest.raises(KeyError, match='Missing required columns'):
        collector.get_stn_number(df, '서울')


def test_get_stn_number_location_not_exists(stn_meta_df, collector):
    with pytest.raises(ValueError, match='No STN_ID found for location'):
        collector.get_stn_number(stn_meta_df, '춘천')
