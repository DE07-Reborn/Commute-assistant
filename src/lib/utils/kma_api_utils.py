...
class Kma_api_collector:
    ...
    def get_stn_number(self, df, location):
        """
            From stn_meta dataset, extract stn_value by location
            param 
                df : pandas dataframe (stn-metadata)
                location : the location wanted to get stn info
        """
        if not isinstance(df, pd.DataFrame):
            raise TypeError("df must be a pandas DataFrame")
        
        required_cols = {'지역', 'STN_ID'}
        if not required_cols.issubset(df.columns):
            raise KeyError(f"Missing required columns: {required_cols}")

        stn = df[df['지역'] == location]['STN_ID'] 

        if stn.empty :
            raise ValueError(f"No STN_ID found for location: {location}")

        return int(stn.iloc[0])